...
import HallOfSaintsView from './components/HallOfSaintsView';
...
const migrateManager = (manager: any): Manager => {
  return {
    ...
    imageUrl: manager.imageUrl || undefined,
    facepackId: manager.facepackId || undefined,
  };
};

const Sidebar: React.FC<{ activeTab: string; setActiveTab: (tab: any) => void }> = ({ activeTab, setActiveTab }) => {
  const getTabClass = (tabName: string) => {
    return `w-full text-left text-sm font-semibold px-3 py-2.5 rounded-md transition-colors flex items-center gap-3 ${activeTab === tabName ? 'bg-blue-600/30 text-blue-200 border border-blue-500/50' : 'text-gray-400 hover:bg-gray-700/50 hover:text-gray-200'}`;
  };
    ...
    <nav className="w-56 flex-shrink-0 bg-gray-900/80 border-r border-gray-700/50 flex flex-col p-3">
      <div className="flex-shrink-0 mb-6 mt-1">
        <h1 className="text-2xl font-bold text-white tracking-wider text-center">
          FM <span className="text-blue-400">ARCHIVE</span>
        </h1>
      </div>
      ...
      <div className="flex-1 space-y-1.5">
        ...
        <button onClick={() => setActiveTab('hall-of-saints')} className={getTabClass('hall-of-saints')}>
          <StarIcon className="w-5 h-5" /> Hall of Saints
        </button>
...
const App: React.FC = () => {
  // Common states
  const [activeTab, setActiveTab] = useState<'dashboard' | 'database' | 'all-time-xi' | 'hall-of-saints' | 'matchday' | 'compare' | 'player-cards' | 'growth-history' | 'customise' | 'about'>('dashboard');
  ...
  const [dateFormat, setDateFormat] = useState<DateFormatOption>(() => (localStorage.getItem(DATE_FORMAT_KEY) as DateFormatOption) || 'mm/dd/yyyy');
  const [currency, setCurrency] = useState<CurrencyOption>(() => (localStorage.getItem(CURRENCY_KEY) as CurrencyOption) || 'USD');
  ...
  useEffect(() => {
    ...
    const currentTags = getCustomData<CustomTag>(CUSTOM_TAGS_KEY);
    const saintsTagName = 'Hall of Saints';
    if (!currentTags.some(tag => tag.name === saintsTagName)) {
        const newTag: CustomTag = { id: uuidv4(), name: saintsTagName, color: '#fde047' };
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case 'hall-of-saints':
        return <HallOfSaintsView allPlayers={players} allClubs={allClubs} customTags={customTags} kitDisplayIndices={kitDisplayIndices} onKitClick={handleKitDisplayChange} />;
      case 'matchday':
