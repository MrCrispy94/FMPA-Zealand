...
export const ManagerDetails: React.FC<ManagerDetailsProps> = ({ manager, onSave, onDelete, onToggleFavourite, onAddClub, allHonours, allClubs, saveGames, fmVersions, customIcons, trophyCabinetSettings, facesPath, facepackConfig }) => {
  ...
  const handleSave = () => {
    const managerToSave = { ...editedManager };

    // Check if facepackId has been changed and is not empty
    if (managerToSave.facepackId && managerToSave.facepackId !== manager.facepackId) {
        const imagePath = facepackConfig[managerToSave.facepackId];
        if (imagePath && facesPath) {
            ...
            managerToSave.imageUrl = finalPath;
        }
    }
    
    onSave(managerToSave);
    setIsEditing(false);
  };
  ...
  const handleSyncFacepack = () => {
    ...
    const imagePath = facepackConfig[editedManager.facepackId];
    if (imagePath) {
        const cleanFacesPath = facesPath.endsWith('/') ? facesPath : `${facesPath}/`;
        const finalPath = `${cleanFacesPath}${imagePath}.png`;
        handleChange('imageUrl', finalPath);
    } else {
        alert(`Could not find a mapping for Facepack ID: ${editedManager.facepackId}. Check your config.xml.`);
    }
  };
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6 h-full flex flex-col">
        ...
        <ImageCropModal
            ...
            initialImageSrc={editedManager.imageUrl}
        />
            ...
            <div className="flex items-start gap-4">
                ...
                <div className="text-center">
                     <div className="w-20 h-20 bg-gray-900/50 rounded-lg overflow-hidden flex items-center justify-center border border-gray-700/50">
                        {editedManager.imageUrl ? (
                            <img src={editedManager.imageUrl} alt={displayName} className="w-full h-full object-cover" />
                        ) : (
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-4 bg-gray-900/50 rounded-lg">
            ...
            {isEditing && (
                <div className="col-span-2">
                    ...
                    <div className="flex items-center gap-2">
                        <Input 
                            value={editedManager.facepackId || ''} 
                    ...
                    <table id="season-stats-table" className="w-full text-left">
                        ...
                        <tfoot className="bg-gray-800/60 font-bold text-white border-t-2 border-purple-500/50">
                             <tr>
                                ...
                                <td colSpan={2} className="p-3">{careerTotals.totalGames} games ({careerTotals.winRatio.toFixed(1)}% win)</td>
